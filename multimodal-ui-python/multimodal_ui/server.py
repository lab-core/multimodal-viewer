...
class AngularServer:
    ...
    def start(self):
        handler = lambda *args, **kwargs: AngularRequestHandler(*args, directory=self.static_dir, **kwargs)
        ...
        if not os.path.exists(index_path):
            raise FileNotFoundError(f"index.html not found in static directory: {index_path}")
        
        with socketserver.TCPServer(("", self.port), handler) as httpd:
            print(f"Serving Angular app from {self.static_dir} at http://localhost:{self.port}")
